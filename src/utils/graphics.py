...
def plot_learning_curve(scores, path):

    scores = np.array(scores[1:])[np.array(scores)[1:] > -100]              # Removing outliers
    ...
    plt.xlim([x[0], x[-1]])
    plt.title('Curva de aprendizaje (Media móvil de 100 episodios)', fontsize=18)
    plt.xlabel('Nº de Episodios', fontsize=14)
    plt.ylabel('Score ($\cdot 10^5$)', fontsize=14)
...
def plot_evaluation(env1, env2):
    ...
    axs[0].set_title('Evolución del Sistema', fontsize=18)
    ...
    axs[1].set_title('Acción de Control', fontsize=18)
